...
#include "../include/Layered_Range_tree_traits_2.h"
...
int ps = 20;
  ...
  if (filename) { 
    //READ DATA FROM FILE
    std::ifstream indata; 
    double xin, yin; // variables for input value
    ...
    for(i=1;i<=pow(2,ps);i++)
    {
      a.put_value(first_key++,second_key++);
      intvect.push_back(a);
    }
  ...
  Last_range_tree <Double_data> tree(size);
 ...
 // }
  ...
  vec[0] = pow(2,8);
  vec[1] = pow(2,9);
  from.put_value((int)pow(2,8),(int)pow(2,9));
  vec[0] = pow(2,ps)+pow(2,7);
  vec[1] = pow(2,ps)+pow(2,9);
  to.put_value((int)pow(2,ps)-100,(int)pow(2,ps)-100);
